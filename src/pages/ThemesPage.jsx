...
function ThemeCard({row,isOwner,busy,onAction}){
  const m=row.manifest||{},colors=m.foundations?.colors||{},nav=m.navigation||{},buttons=m.buttons||{},icons=m.icons||{},componentCount=Object.keys(m.component_options||{}).length;
  const locked=row.status!=='DRAFT';
  return <Card title={row.name} subtitle={`${row.key} · v${row.version}`} className="theme-package-card">
    <div className="theme-card-meta"><Badge tone={tone(row.status)}>{row.status}</Badge>{(row.supported_apps||[]).map(app=><Badge key={app}>{appLabel(app)}</Badge>)}{componentCount>0&&<Badge>{componentCount} configurable components</Badge>}</div>
    <div className="theme-mini-preview" style={{'--theme-primary':colors.primary||'#0a84ff','--theme-bg':colors.background||'#f2f2f7','--theme-surface':colors.surface||'#ffffff','--theme-text':colors.text||'#111111'}}>
      ...
      <div className="theme-token-summary"><span>Navigation</span><strong>{nav.mobile||'standard'}</strong><span>Buttons</span><strong>{buttons.primary||'solid'}</strong><span>Icons</span><strong>{icons.pack||'LUKE_OUTLINE'} · {icons.size||24}px</strong><span>Typography</span><strong>{m.typography?.preset||'SYSTEM_MINIMAL'}</strong><span>Components</span><strong>{componentCount?`${componentCount} selectable groups`:'Package defaults only'}</strong></div>
    </div>
...
function PackagePreview({value}){const options=value.manifest?.component_options||{};return <div className="theme-package-review"><span>Package</span><strong>{value.name||'Unnamed'} · v{value.version||'—'}</strong><span>Key</span><strong>{value.key||'—'}</strong><span>Apps</span><strong>{(value.supported_apps||[]).map(appLabel).join(' · ')||'—'}</strong><span>Schema</span><strong>v{value.manifest?.schema_version||'—'}</strong><span>Mobile navigation</span><strong>{value.manifest?.navigation?.mobile||'—'}</strong><span>Selectable components</span><strong>{Object.keys(options).length||'None'}</strong></div>}